...
void Break_nt_Matrix_n_t_components(bool computeInverse, const MAT& mat_nt, double& scalar_nn, VECm1& vec_nt, VECm1& vec_tn, MATm1& mat_tt, MATm1& mat_tt_condensed, MATm1& mat_tt_condensed_Inv)
{
...
bool EigenValuesEigenVectors_PositiveSymMatrix(const MAT & mat, MAT & eigenVecsRowWise, VEC & eigenValues)
{
	...
	if (fabs(c) < tol)
	{
		...
		{
			lambda0 = a, lambda1 = b;
			eigenVecsRowWise[0][0] = 1.0; eigenVecsRowWise[0][1] = 0.0;
			eigenVecsRowWise[1][0] = 0.0; eigenVecsRowWise[1][1] = 1.0;
		}
		else
		{
			eigenVecsRowWise[0][0] = 0.0; eigenVecsRowWise[0][1] = 1.0;
			eigenVecsRowWise[1][0] = 1.0; eigenVecsRowWise[1][1] = 0.0;
		}
	}
	else
	{
		...
		lambda0 = 0.5 * (apb + sqrt_delta), lambda1 = 0.5 * (apb - sqrt_delta); // decreasing order
		double theta = atan(c / (lambda0 - b));
		double costheta = cos(theta), sintheta = sin(theta);
		eigenVecsRowWise[0][0] = costheta; eigenVecsRowWise[0][1] = sintheta;
		eigenVecsRowWise[1][0] = -sintheta; eigenVecsRowWise[1][1] = costheta;
	}
	return true;
#endif
#if DiM3

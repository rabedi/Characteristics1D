#ifndef PHY_MACROS__H
#define PHY_MACROS__H
...
// these flags force the code IO (for simplex geometry + polynomial data / other futher data to be ASCII binary)
...
#else
#define READ_CIO_INTEGER(in, dat, isBinary) 		{ if (isBinary) in.read((char*)&dat, sizeof(int)); else in >> dat; }
#endif
#endif
...
#else
#define READ_CIO_DOUBLE(in, dat, isBinary) 		{ if (isBinary) in.read((char*)&dat, sizeof(double)); else in >> dat; }
#endif
#endif
...
#else
#define WRITE_CIO_INTEGER(out, dat, isBinary) 		{ if (isBinary) out.write((char*)&dat, sizeof(int)); else out << dat; }
#endif
#endif
...
#else
#define WRITE_CIO_DOUBLE(out, dat, isBinary) 		{ if (isBinary) out.write((char*)&dat, sizeof(double)); else out << dat; }
#endif
#endif
...
#else
#define WRITE_CIO_TAB(out, isBinary) 			{ if (!isBinary) out << '\t'; }
#endif
#endif
...
#else
#define WRITE_CIO_EOL(out, isBinary) 			{ if (!isBinary) out << '\n'; }
#endif
#endif

template <class T>
inline void write_cio_midline(std::ostream& out, const T&dat, bool isBinary = true)
{
#if FORCE_CIO_BINARY 
...
template <class T>
inline void write_cio_endline(std::ostream& out, const T&dat, bool isBinary = true)
{
#if FORCE_CIO_BINARY 
	out.write((char*)&dat, sizeof(T));
	return;
...
template<class T>
bool fromString(const string& name, T& dat)
{	
...
template<class Ty>
istream& binary_read(istream& in, string& buf, Ty& var)
{
	if (!in.read((char*)&var, sizeof(Ty)))
	{
		stringstream ss;
		ss << "Error occured during binary file read::number of bytes read[" << in.gcount() << "]\n";
		THROW(ss.str().c_str());
	}
	buf = to_string(var);
	return in;
...
template<class Ty>
ostream& binary_write(ostream& out, Ty var)
{
	if (!out.
		write((char*)&var, sizeof(Ty)))
	{
...
// reads the next input, if it's not the write type, it goes back and returns false
// if the next input starts with character # the entire line is read

// readUntilSuccessful 
//	true	reads until it finds specific type requested and skips all incorrect formats and comments
//			only returns false if until the end of the file the particular type cannot be found
//	false	reads ONLY one value:
//				if it has correct format		=>		returns true
//				if NOT							=>		puts back the wrong value into string		
// comment ends with precedingEndCommend commentChar, for example for commentChar = '#' and precedingEndCommend = '\', end of comment would be \#
// 0 means unacceptable format
// -1 means end of file (fail to read)
// 1 means successful read
template <class T>
inline int ReadSpecificType(istream& in, T& val, string &buf, bool readUntilSuccessful = false, char commentChar = '#', string precedingEndCommend = "\\#")
{
...
{	\
	string bufs;	\
	if (ReadSpecificType(iFile, var, bufs) == 0) { cout << "buf\n" << buf << '\n'; THROW("invalid file format for reading integer\n");} \
...
{	\
	string tmp; \
	string bufs;	\
	if (ReadSpecificType(iFile, tmp, bufs) == 0) { cout << "buf\n" << buf << '\n'; THROW("invalid file format for reading boolean\n");} \
	if ((tmp[0] == 'y') || (tmp[0] == '1'))					\
		var = true;				\
...
{	\
	string bufs;	\
	if (ReadSpecificType<double>(iFile, var, bufs) == 0) { cout << "buf\n" << buf << '\n'; THROW("invalid file format for reading double\n");} \
	strcpy(buf, bufs.c_str());	\
}		\
...
{	\
	if (ReadSpecificType(iFile, var, buf) == 0) { cout << "buf\n" << buf << '\n'; THROW("invalid file format for reading double\n");} \
}		\

#define READ_OSTRING(iFile, buf, var) 						\
{	\
	string bufs, s;	\
	if (ReadSpecificType(iFile, s, bufs) == 0) { cout << "buf\n" << buf << '\n'; THROW("invalid file format for reading double\n");} \
	strcpy(buf, bufs.c_str());\
	strcpy(var, s.c_str());\
}		\

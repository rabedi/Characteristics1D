...
typedef enum{ appendF, renameF, copyF, moveF, removeD, makeD, removeF } op; //F:File, D:Directory
...
{
	string prePath; getCurrWorkDir(prePath);
	...
	struct stat s_buf; //ref: http://pubs.opengroup.org/onlinepubs/007908799/xsh/sysstat.h.html
	...
	{	//sometimes files aren't properly determined so this is a fail safe check
...
inline void MakeDir(string folderName)
{
#if defined(_MSC_VER)
//	struct _stat dir_stat;
	...
	if (stat(folderName.c_str(), &dir_stat) == -1)//the directory doesn't exist
